...
- **Fixed (macOS)**: files created while running under `sudo` (`config.toml`, `logs/run.log`) are handed back to your user, so a later non-`sudo` launch or config save still works.
- **Fixed**: the macOS release build script mis-resolved a relative output directory, which broke the automated macOS release zip.
...
- **追加**: 記号キーのキー名(両OS共通): `LBRACKET` `RBRACKET` `SEMICOLON` `QUOTE` `COMMA` `PERIOD` `SLASH` `BACKSLASH` `MINUS` `EQUALS` `GRAVE`(他のキーと同様、US配列の物理位置)。
- **修正(macOS)**: `sudo`で起動中に作成されたファイル(`config.toml`、`logs/run.log`)の所有者を元のユーザーに戻すようにした。その後`sudo`なしで起動したり設定を保存したりしても問題なくなる。
- **修正**: macOS用リリースビルドスクリプトが相対パスの出力先を誤って解決し、自動生成のmacOS用zipが壊れていた問題を修正。
