...
- Punctuation (US-layout positions): `"LBRACKET"` `[` `"RBRACKET"` `]` `"SEMICOLON"` `;` `"QUOTE"` `'` `"COMMA"` `,` `"PERIOD"` `.` `"SLASH"` `/` `"BACKSLASH"` `\` `"MINUS"` `-` `"EQUALS"` `=` `"GRAVE"` `` ` ``
...
- 記号キー(US配列の物理位置): `"LBRACKET"` `[` `"RBRACKET"` `]` `"SEMICOLON"` `;` `"QUOTE"` `'` `"COMMA"` `,` `"PERIOD"` `.` `"SLASH"` `/` `"BACKSLASH"` `\` `"MINUS"` `-` `"EQUALS"` `=` `"GRAVE"` `` ` ``
